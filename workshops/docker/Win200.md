...
- Go to the following URL: ( optionally you could use: `192.168.99.100:5600/em` )

```
http://locahost:5600/em
```
...
```
docker cp /c/<YOUR_HOME>/AlphaOfficeSetup/dark_blue.jpg alphaofficeui:/pipeline/source/public/Images
```
...
- Open up a new browser tab and this time use the IP Address of the docker-machine VM (in this example `192.168.99.100`). We're just showing that we can use the Linux VM's IP Address as well as `localhost`. You can obtain the Linux VM's IP by **typing** in:
